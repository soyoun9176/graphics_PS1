...
def create_base_head(parent_joint, head_color=(255, 150, 20, 255), 
                     blush_color=(255, 215, 0, 255), 
                     mouth_color=(255, 255, 255, 255)):
    """
    Creates a common head base with face, eyes (regular and alt), and features.
    Returns: head_root, parts, head_eye_parts, head_alt_eye_parts
    """
    parts = []
    ...
    head_cheeks.local_transform = Mat4.from_translation(Vec3(0, -0.2, 0))
    ...
    head_eye_parts = {
        "eye_l": eye_l_part, "pupil_l": pupil_l_part,
        "eye_r": eye_r_part, "pupil_r": pupil_r_part,
    }
    head_alt_eye_parts = {
        "alt_eye_l1": eye_fx_l1_part, "alt_eye_l2": eye_fx_l2_part,
        "alt_eye_r1": eye_fx_r1_part, "alt_eye_r2": eye_fx_r2_part,
    }

    # 5. blush
    blush_mesh = Sphere(16, 16, 1.0)
    _solid_color(blush_mesh, blush_color)
    ...
    parts.append(Part(blush_l, blush_mesh, Mat4.from_scale(Vec3(0.12, 0.06, 0.05))))
    ...
    parts.append(Part(blush_r, blush_mesh, Mat4.from_scale(Vec3(0.12, 0.06, 0.05))))
...
def create_pungpung_head(parent_joint, head_color=(255, 145, 15, 255)):
    """
    Creates a Pungpung head: Base head + Leaf ornament.
    """
    head_root, parts, eyes, alt_eyes = create_base_head(parent_joint, head_color)
    
    # Add leaf ornament (gulggokji)
    ...
    return head_root, parts, eyes, alt_eyes
